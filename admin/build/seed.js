...
import { companionTemplate, nonplayerTemplate, playerTemplate, } from './characterTemplates.js';
...
await Promise.all([
    Promise.all(users.map((user) => app.firestore().doc(`/users/${user.id}`).set(user))),
    Promise.all(players.map((p) => app.firestore().doc(`/characters/${p.id}`).set(p))),
    Promise.all(rolls.map((roll) => app.firestore().collection(`/rolls`).add(roll))),
    Promise.all(companions.map((p) => app.firestore().doc(`/characters/${p.id}`).set(p))),
]);
